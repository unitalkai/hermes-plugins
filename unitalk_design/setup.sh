#!/usr/bin/env bash
#
# Install the unitalk_design plugin (Iris/Lumi graphic & motion co-pilot) into a
# Hermes profile. Idempotent. Run inside the instance:
#
#   ./setup.sh          # installs for the 'iris' profile (default)
#   ./setup.sh <profile>   # e.g. ./setup.sh lumi
#
# This script bakes in EVERY lesson we learned provisioning the plugins:
#
...
==> Done. NOW RESTART THE GATEWAY so the runtime rescans + registers the plugin.
    Then, in a fresh session, /plugins should show '$PLUGIN' and the design_* tools.

    Per-user prerequisites on this profile:
      • env: IRIS_WORKSPACE_URL (Convex .site) + LITELLM_KEY_ID
             (that key's /key/info metadata.userId == the signed-in Clerk id)
      • model: gpt-5.6-luna (so the agent tool_searches the deferred design_* tools)
EOF
